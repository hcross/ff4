...
def _is_fail_closed(g: dict) -> bool:
...
def render_variant_gaps(analyses: list[dict], table_count: int) -> str:
    ...
    for a in analyses:
        ...
        out.append(f"\n## {a['short_name']} (`{a['patch_id']}`, CRC32 `{a['crc32']}`)\n")
        out.append(f"\n{len(a['gated'])} of {table_count} live table entries gated "
                   f"({len(proven)} proven invalidations, {len(unresolved)} fail-closed); "
        ...
        out.append("\n### Fail-closed (unresolved range -- pin the range to un-gate)\n\n"
                   "| ID | Routine | Level |\n|---|---|---|\n")
        for g in unresolved:
            out.append(f"| {g['id']} | `{g['name']}` | {g['level']} |\n")
    return "".join(out)
...
def main(argv: list[str] | None = None) -> int:
    ...
    rendered_gaps = render_variant_gaps(analyses, len(table_pcs))
    ...
    if args.check:
        ...
        if rendered != PROFILES_C.read_text():
            ...
            return 1
        if not GAPS_MD.is_file() or rendered_gaps != GAPS_MD.read_text():
            sys.stderr.write(f"drift: {GAPS_MD} does not match a fresh generation\n")
            return 1
        print(f"rom_profiles.c + VARIANT_GAPS.md up to date "
              f"({len(analyses)} variant profiles)")
        return 0
    ...
    if not args.patch_id:
        ...
        GAPS_MD.write_text(rendered_gaps)
        print(f"\nwrote {PROFILES_C}")
        print(f"wrote {GAPS_MD}")
    else:
        print("\n(single-patch run: rom_profiles.c / VARIANT_GAPS.md NOT rewritten "
              "— run without --patch-id to regenerate all profiles)")
    return 0
