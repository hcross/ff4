...
- **[registry/VARIANT_GAPS.md](registry/VARIANT_GAPS.md)** — GENERATED by
  `registry/patch_impact.py` (never hand-edit): per-variant dispatch gaps —
  which ported routines do NOT run natively under each translation-patch
  image, split proven-invalidation vs fail-closed-unresolved, with the
  remediation path. Reviewed at every new dispatch hook (WF-DECOMP step 6).
